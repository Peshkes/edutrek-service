...
public class  ContactsFilterSpecifications <E extends AbstractContacts>{
    ...
    public Specification<E> hasName(String name) { return (r, q, cb) -> cb.like(r.get("contactName"), "%" + name + "%");    }

    public Specification<E> hasPhone(String phone) {
        return (r, q, cb) -> cb.like(r.get("phone"), "%" + phone + "%");
    }

    public Specification<E> hasEmail(String email) { return (r, q, cb) -> cb.like(r.get("email"), "%" + email + "%"); }

    public Specification<E> hasTargetCourseId(UUID targetCourseId) { return (r, q, cb) -> cb.equal( r.get("targetCourseId") ,targetCourseId); }
